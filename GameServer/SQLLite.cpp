...
CSQLite::CSQLite() : m_pDB(NULL), m_stmt(NULL), m_handleOpenCheck(false), m_flag(0)
{
}
...
bool CSQLite::Connect(const char* strConnect)
{
	if (IsOpen())
		return false;
...
bool CSQLite::Query(const char* strQuery)
{
...
bool CSQLite::QueryExec(const char* strQuery)
{
...
void CSQLite::AllocHandle()
{
	m_handleOpenCheck = true;
}

void CSQLite::FreeHandle()
{
	m_handleOpenCheck = false;
	if (m_stmt)
	{
...
bool CSQLite::GetData(int nFieldNum, char* result)
{
...
bool CSQLite::GetData(int nFieldNum, unsigned char* result)
{
...
bool CSQLite::GetData(int nFieldNum, short* result)
{
	if (m_stmt == NULL) return 0;
...
bool CSQLite::GetData(int nFieldNum, unsigned short* result)
{
	if (m_stmt == NULL) return false;
...
bool CSQLite::GetData(int nFieldNum, int* result)
{
	if (m_stmt == NULL) return false;
...
bool CSQLite::GetData(int nFieldNum, unsigned int* result)
{
	if (m_stmt == NULL) return false;
...
bool CSQLite::GetData(int nFieldNum, unsigned long long* result)
{
	if (m_stmt == NULL) return false;
...
bool CSQLite::GetData(int nFieldNum, long long* result)
{
	if (m_stmt == NULL) return false;
...
bool CSQLite::GetData(int nFieldNum, double* result)
{
	if (m_stmt == NULL) return false;
...
bool CSQLite::GetData(int nFieldNum, char* const result, unsigned int nSize) // IN: limit OUT : actual size
{
	if (m_stmt == NULL) return false;
...
int CSQLite::GetDataBytea(int nFieldNum, unsigned char* const result, unsigned int nSize)
{
	unsigned int length = sqlite3_column_bytes(m_stmt, nFieldNum);
...
int CSQLite::SetDataBytea(const unsigned char* bintext, size_t binlen, unsigned char* rettext, size_t retlen)
{
	return BinaryToText(bintext, binlen, rettext, retlen);
}

void CSQLite::DisplayErrorMsg()
{
